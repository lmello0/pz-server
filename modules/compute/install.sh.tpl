...
# --- Skip the slow install entirely if this is the baked AMI ---
# /etc/pz-server-baked is written by packer/bake.sh once SteamCMD, the
# game, and the JVM heap fix are already in place. On a baked image this
# whole block (multiverse/apt/steamcmd/download/patch) is unnecessary -
# it's already done - which is what turns a ~10 minute boot into roughly
# 30-60 seconds. Deploying from stock Ubuntu (no custom_ami_id set) still
# works exactly as before; this branch just does the full install itself.
if [ -f /etc/pz-server-baked ]; then
  echo "baked AMI detected - skipping OS/SteamCMD/game install"
else
  echo "stock image detected - running the full install (several minutes)"
  ...
  # on its own, but this keeps the logs clean.
  ...
  # by hand across several rebuilds - retry rather than guess at the cause.
  STEAM_OK=0
  ...
  # See the systemd block below for why this exists.
...
# attaches asynchronously, shortly after the instance boots, so the
# device may not exist yet when this script starts. Wait for it.
DEVICE=/dev/nvme1n1
...
# Written here rather than baked into the AMI because ExecStart carries
# this deployment's admin password - baking it in would mean the AMI
# itself contains a live credential.
